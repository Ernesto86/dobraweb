...
if (!isset($_POST['txt_tipo']) or !isset($_POST['id_cliente']) or !isset($_POST['idfactura']) or !isset($_POST['txt_total_devol']) or !isset($_POST['txt_ing_fecha']) or !isset($_POST['txt_cli_concepto'])) {
    die('{"err":"3"}');
}
...
require '../clases/cls_ven_devolucion_venta.php';
$obj = new cls_ven_devolucion_venta;
...
if ($saldo_client != $sald_det) {
    die('{"err":"5"}');
}

$obj->arr_val_devuelto = $_POST['txt_devuelto'];
$obj->arr_total_devol = $_POST['txt_abono'];
$obj->arr_val_cantidad = $_POST['txt_cantidad'];

$ingr_cab = floatval(array_sum($obj->arr_val_devuelto));
$ingr_det = floatval(array_sum($obj->arr_total_devol));
$ingt_cant = floatval(array_sum($obj->arr_val_cantidad));

$obj->total_devol = floatval($_POST['txt_total_devol']);
$obj->subtotal_devol = floatval($_POST['txt_total_devol']);

if ($ingr_cab <= 0 or $obj->total_devol <= 0 or $ingt_cant <= 0) die('{"err":"5"}');

if ($obj->total_devol != $ingr_det) die('{"err":"5"}');
...
$obj->s_det_concepto($_POST['txt_cli_concepto']);

$obj->s_id_bodega($_POST['id_bodega']);
$obj->s_id_vendedor($_POST['id_empleado']);
...
$obj->fn_ven_guardar_devolucion_venta(); ?>